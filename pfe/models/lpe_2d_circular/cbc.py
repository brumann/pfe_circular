...
class CBC:
    ...
    .. math::
       ...
       + \frac{\rho_0}{c_0^2}(\mathbf{u}_0\cdot\mathbf{n})
       (\mathbf{u}_0\cdot\boldsymbol{\tau})\overline{\psi}\frac{\partial\phi}{\partial n}
       \,\mathrm{d}\Gamma \;,
    ...
    def terms(self, model, e):
        ...
        g = self.source.get_value(e, u, xr)
        u0tau = u0 * tau[:, 0] + v0 * tau[:, 1]
        u0n = u0 * n[:, 0] + v0 * n[:, 1]
        r = xr[1]

        Ke = -phi.T @ (
            (weights * rho0 * u0n * u0tau / c0 ** 2 * r)[:, None] * dphidtau
        ) - phi.T @ ((weights * rho0 * 1j * omega / c0 * r)[:, None] * phi)
        Fe = -phi.T @ ((weights * rho0 * (1 - (u0n / c0) ** 2) * r) * g)
        return (Ke, Fe)
