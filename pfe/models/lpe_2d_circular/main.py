...
class Main:
    ...
    def terms(self, model, e):
        ...
        Ke = (
            D0phiDt.T.conj() @ ((weights * rho0 * r * 2 * np.pi / c0 ** 2)[:, None] * D0phiDt)
            - dphidx.T.conj() @ ((weights * rho0 * r * 2 * np.pi)[:, None] * dphidx)
            - dphidy.T.conj() @ ((weights * rho0 * r * 2 * np.pi)[:, None] * dphidy)
            - m**2 * phi.T.conj() @ ((weights * rho0 / r * 2 * np.pi )[:, None] * phi)
        )
        return Ke
