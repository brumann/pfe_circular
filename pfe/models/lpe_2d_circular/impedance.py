...
class Impedance:
    ...
    def terms(self, model, e):
        """Compute the element matrix for a single element (only Z2 is accepted)
        ...
        r2 = model.parameters["r2"].get_value(e, u, xr)
        ...
        Ke = -D0phiDt.T.conj() @ (
                (weights * rho0 / c0 * inv_Z * 1j / omega * r2 * 2 * np.pi)[:, None] * D0phiDt
        )
